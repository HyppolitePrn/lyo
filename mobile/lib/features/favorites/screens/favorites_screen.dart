    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        ...
        body: switch (notifier.status) {
          FavoritesStatus.loading || FavoritesStatus.idle =>
...
class _EmptyState extends StatelessWidget {
  const _EmptyState({required this.icon, required this.label, required this.textSub});
  final IconData icon;
  final String label;
  final Color textSub;
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text(label, style: TextStyle(color: textSub, fontSize: lyoBody1)),
        ],
      ),
    );
  }
...
class _TrackList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (tracks.isEmpty) {
      return _EmptyState(
          icon: Icons.music_off_outlined, label: 'No favorite tracks yet', textSub: textSub);
    }
...
class _StreamList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (streams.isEmpty) {
      return _EmptyState(
          icon: Icons.radio, label: 'No favorite streams yet', textSub: textSub);
    }
    return ListView.separated(
      padding: const EdgeInsets.all(lyoGapM),
